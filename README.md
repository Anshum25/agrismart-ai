...
[![Groq](https://img.shields.io/badge/GenAI-Llama%203.3%20%2B%20Whisper-F55036)](https://groq.com/)
...
5. **Answers follow-up questions by voice** (Groq Whisper + Llama 3.3, grounded in the diagnosis).
...
```text
 ...
 photo ───►  │ quality gate → ONNX Runtime Web (worker) → Grad-CAM → severity     │ ◄── works offline
             ...
             │ /advice    Groq Llama 3.3 → offline pack → curated knowledge base  │
             │ /voice/ask Groq Whisper + Llama 3.3                                │
...
│   ├── public/samples/       6 real held-out test images
...
│   ├── export_onnx.py        Keras → ONNX (int8) + parity report
...
ResNet50 (ImageNet) → GAP → Dense(256) → Dropout(0.4) → Dense(38), fine-tuned on PlantVillage (color) with a
stratified 70/15/15 split and two-phase unfreezing (`model/train.py`).

| Metric | Value |
|---|---|
| Test accuracy (PlantVillage held-out split) | **98.96%** (see `frontend/public/model/reports/`) |
| Classes | 38 (14 crops) |
| Shipped format | ONNX, static int8 (QDQ), ~25 MB |
